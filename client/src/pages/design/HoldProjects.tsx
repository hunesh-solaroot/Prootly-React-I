...
import { SortConfig, TableFilters, TextMode } from '@shared/types';
...
export default function DesignWorkflowPage({ tableType, pageTitle }: DesignWorkflowPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log(`🚀 HoldProjects: Starting data fetch for tableType: "${tableType}"`);
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        console.log(`✅ HoldProjects: Received response with ${response.data.length} projects and ${response.headers.length} headers`);
        ...
        console.log(`✅ HoldProjects: Setup complete with ${initialWidths.length} columns`);
        
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : "Failed to load project data";
        console.error('❌ HoldProjects: Error during fetch:', err);
  ...
  const handleColumnTextModeChange = (column: string, mode: TextMode | null) => {
